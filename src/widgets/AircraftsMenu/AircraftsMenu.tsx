...
import { sortObjByKeyAlphabetically } from "@/utils/sort";
  ...
  return (
    <div className={defaultClassNames}>
      <ul className={style.ManufacturersList}>
        {Object.keys(sortObjByKeyAlphabetically(acftByManufacturer)).map(
          (manufacturer) => {
            return (
              <li
                className={style.Manufacturer}
                key={`manufacturer-${slugify(manufacturer)}`}
              >
                <input
                  type="checkbox"
                  id={`manufacturer-${slugify(manufacturer)}`}
                />
                ...
                <ul className={style.AircraftsList}>
                  {acftByManufacturer[manufacturer].map((aircraft) => {
                    return (
                      <li
                        className={style.Aircraft}
                        key={`acft-${aircraft.key}`}
                      >
